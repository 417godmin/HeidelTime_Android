 ...
 * {@link CAS#createArrayFS(int length)} or new FSArray(aJCas, length)
 */
public interface ArrayFS<E extends FeatureStructure> extends CommonArrayFS<E> {
   ...
   *          index
   * @param <U>
   *          The class of the item being obtained by the get
   ...
   *          The number of elements to copy.
   * @param <T>
   *          the class of the array being copied into
   * @exception ArrayIndexOutOfBoundsException
   *              If <code>srcOffset &lt; 0</code> or <code>length &gt; size()</code> or
   *              <code>destOffset + length &gt; destArray.length</code>.
   */
  <T extends FeatureStructure> void copyFromArray(T[] src, int srcOffset, int destOffset,
          int length) throws ArrayIndexOutOfBoundsException;

  /**
   * Creates a new array the this array is copied to. Return type is FeatureStructure to be
