    ...
    void switchClassLoader(ClassLoader newClassLoader, boolean wasLocked) {
      ...
      if (!(newClassLoader instanceof UIMAClassLoader)) {
        UIMAFramework.getLogger().log(
                org.apache.uima.util.Level.FINE,
                "Calling switchClassLoader with a classloader of type ["
                        + newClassLoader.getClass().getName()
                        + "] that is not a UIMAClassLoader may cause JCas wrappers to be loaded from the wrong classloader."
        );
      }
