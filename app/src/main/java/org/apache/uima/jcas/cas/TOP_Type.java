...
public class TOP_Type {
  ...
  public static org.apache.uima.jcas.cas.TOP_Type jcasType;//新增兼容逻辑
  ...
  protected TOP_Type(JCas jcas, Type casType, boolean installGenerator) {
    ...
    TOP_Type.jcasType = this;

    // ====== 验证逻辑开始 ======
    try {
      System.out.println("### PATCHED TOP_Type constructor loaded by: "
              + TOP_Type.class.getClassLoader());

      java.lang.reflect.Field jField = TOP_Type.class.getDeclaredField("jcasType");
      System.out.println("### jcasType found, type = " + jField.getType().getName()
              + ", value = " + jField.get(this));

    } catch (Throwable t) {
      System.err.println("!!! Failed to find jcasType field in TOP_Type !!!");
      t.printStackTrace();
    }
    // ====== 验证逻辑结束 ======
